...
/// clause.
///
/// @tparam  E  Enum whose values should be treated like bit-flags.
/// @tparam  T  Underlying integer type used to represent the set values.
...
public:
   ...
   ///
   /// @param[in]  start_value  Bit/flag to set.
   /// @since 0.8, 11.11.2016
   ...
   /// flags to set.
   /// @param[in]  start_set  List of bits/flags to set.
   /// @since 0.8, 11.11.2016
   ...
   /// @param[in]  value  Single bit/flag to set.
   /// @returns  Self.
   /// @since 0.8, 11.11.2016
   EnumFlags& operator =( E value) noexcept;

   /// Assign a list of enum values.
   /// @param[in]  enum_set  List of enum values to set.
   /// @returns  Self.
   /// @since 0.8, 11.11.2016
   ...
   /// @param[in]  value  Single enum value to compare against.
   /// @returns  \c true if only this bit/flag is set.
   /// @since  0.8, 13.11.2016
   bool operator ==( E value) const noexcept;

   /// Compares against a list of bits/flags.
   /// @param[in]  enum_set  List of values to compare against.
   /// @returns  \c true if exactly the specified bits/flags are set.
   /// @since  0.8, 13.11.2016
   bool operator ==( std::initializer_list< E> enum_set) const noexcept;

   /// Compares against the set of another object.
   /// @param[in]  enum_flags  Other object to compare against.
   /// @returns  \c true if the two objects have exactly the same bits/flags set.
   /// @since  0.8, 13.11.2016
   bool operator ==( const EnumFlags& enum_flags) const noexcept;

   /// Compares against a single bit/flag.
   /// @param[in]  value  Single enum value to compare against.
   /// @returns  \c true if more/other bits/flags are set.
   /// @since  0.8, 13.11.2016
   bool operator !=( E value) const noexcept;

   /// Compares against a list of bits/flags.
   /// @param[in]  enum_set  List of values to compare against.
   /// @returns  \c true if at least one other bit/flag is set.
   /// @since  0.8, 13.11.2016
   bool operator !=( std::initializer_list< E> enum_set) const noexcept;

   /// Compares against the set of another object.
   /// @param[in]  enum_flags  Other object to compare against.
   /// @returns  \c true if the two objects differ in at least one bit/flag.
   /// @since  0.8, 13.11.2016
   bool operator !=( const EnumFlags& enum_flags) const noexcept;
   ...
   /// given bit/flag set.
   ///
   /// @param[in]  or_value  Additional enum value/flag to set.
   /// @returns  New object with all bits set as described.
   ...
   /// @param[in]  exclusive_or_value  Bit/flag to logically exclusive-or with.
   /// @returns  Self.
   /// @since 0.8, 13.11.2016
   EnumFlags& operator ^=( E exclusive_or_value) noexcept;

   /// Logical exclusive-or with a list of flags/bits.
   /// @param[in]  enum_set  Bits/flags to logically exclusive-or with.
   /// @returns  Self.
   /// @since 0.8, 13.11.2016
   EnumFlags& operator ^=( std::initializer_list< E> enum_set) noexcept;

   /// Logical exclusive-or with the flags/bits from another object.
   /// @param[in]  enum_flags  Other object to logically exclusive-or the
   ///                         flags/bits with.
   /// @returns  Self.
   /// @since 0.8, 13.11.2016
   EnumFlags& operator ^=( const EnumFlags& enum_flags) noexcept;
   ...
   /// @param[in]  and_value  Single bit/flag to check if it is set.
   /// @returns  \c true if the specified bit/flag is set.
   /// @since  0.8, 11.11.2016
   ...
   /// @param[in]   ef  Object to dump the bit-set of.
...
private:
   ...
   /// @param[in]  enum_value  Enum value to compute the bit-mask for.
   /// @returns  Bit-mask of the enum value.
   /// @since  0.8, 11.11.2016
...
template< typename E, typename T>
   EnumFlags< E, T>::EnumFlags( const E start_value) noexcept:
...
template< typename E, typename T>
   EnumFlags< E, T>& EnumFlags< E, T>::operator =( const E value) noexcept
{
...
template< typename E, typename T>
   bool EnumFlags< E, T>::operator ==( const E value) const noexcept
{
...
template< typename E, typename T>
   EnumFlags< E, T> EnumFlags< E, T>::operator |( const E or_value) noexcept
{
   auto  copy( *this);
   copy |= or_value;
   return copy;
...
template< typename E, typename T>
   EnumFlags< E, T>&
      EnumFlags< E, T>::operator &=( const E and_value) noexcept
{
...
template< typename E, typename T>
   void EnumFlags< E, T>::clear( const E clear_value) noexcept
{
...
template< typename E, typename T>
   bool EnumFlags< E, T>::operator &( const E and_value) const noexcept
{
...
///
/// @param[in]  lhs
///    First enum value to store.
/// @param[in]  rhs
///    Second enum value to store.
/// @return
///    An EnumFlags object where the two flags are set.
/// @since
///    1.8.0, 26.07.2018
template< typename E, typename T = typename std::underlying_type< E>::type>
   EnumFlags< E, T> operator |( const E lhs, const E rhs)
{
