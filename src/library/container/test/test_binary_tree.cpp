...
bool checkTree3( BinaryTree< int>& int_tree)
{
   ...
   CHECK_RETURN( !assigned.empty());
   CHECK_RETURN( assigned.size() == 3);

   // also make one iteration test with the assigned copy
   {
      auto  tree_iter = assigned.begin();

      CHECK_RETURN( tree_iter != assigned.end());
      CHECK_EQUAL_RETURN( *tree_iter, 13);
   ...
   {
      auto  result = int_tree.insert( 13);
      BOOST_REQUIRE( !result.second);
      BOOST_REQUIRE( result.first != int_tree.end());
      BOOST_REQUIRE_EQUAL( *result.first, 13);
   } // end scope

   {
      auto  result = int_tree.insert( 42);
      BOOST_REQUIRE( !result.second);
      BOOST_REQUIRE( result.first != int_tree.end());
      BOOST_REQUIRE_EQUAL( *result.first, 42);
   } // end scope

   {
      auto  result = int_tree.insert( 4711);
      BOOST_REQUIRE( !result.second);
      BOOST_REQUIRE( result.first != int_tree.end());
      BOOST_REQUIRE_EQUAL( *result.first, 4711);
   } // end scope
...
/// Checks that the contents of a tree are as expected.
/// Always call this function with a binary tree that contains the values
/// 13, 42 and 4711 in ascending order.
///
/// @param[in]  int_tree  The tree object to tet the contents of.
...
bool checkTree3const( const BinaryTree< int>& int_tree)
{
...
BOOST_AUTO_TEST_CASE( empty_tree_tests)
{
   ...
   BOOST_REQUIRE_EQUAL( int_tree.size(), 0);
...
BOOST_AUTO_TEST_CASE( single_entry_tests)
{
   ...
   BOOST_REQUIRE( !int_tree.empty());
   BOOST_REQUIRE_EQUAL( int_tree.size(), 1);
...

   {
      ...
      BOOST_REQUIRE_EQUAL( *tree_iter, 42);
...
BOOST_AUTO_TEST_CASE( two_left_tests)
{
   ...
   BOOST_REQUIRE( !int_tree.empty());
   BOOST_REQUIRE_EQUAL( int_tree.size(),  2);
...

   {
      ...
      BOOST_REQUIRE( tree_iter != int_tree.end());
      BOOST_REQUIRE_EQUAL( *tree_iter, 13);
...
BOOST_AUTO_TEST_CASE( two_right_tests)
{
...

   {
      ...
      ++tree_iter;
      BOOST_REQUIRE( tree_iter != int_tree.end());
      BOOST_REQUIRE_EQUAL( *tree_iter, 4711);
...
BOOST_AUTO_TEST_CASE( three_balanced_tests)
{
   ...
   BOOST_REQUIRE( checkTree3const( int_tree));
...
BOOST_AUTO_TEST_CASE( erase_middle)
{
...

   {
      ...
      for (int i = 0; i < 7; ++i)
      {
         ...
         BOOST_REQUIRE( tree_iter != int_tree.cend());
         BOOST_REQUIRE_EQUAL( *tree_iter, i);
         ++tree_iter;
      } // end for
   } // end scope
...
BOOST_AUTO_TEST_CASE( move_ctor)
{
   ...
   BinaryTree< int>  int_tree( std::move( bti));

   BOOST_REQUIRE( int_tree.check());
   BOOST_REQUIRE_EQUAL( int_tree.size(), 10);
