...
/// %btree with each of the permutation.<br>
...
/// @param[in]  numbers  The numbers in the next sequence to test.
...
bool test_permutation( const std::vector< int>& numbers)
{
...

   {
      ...
      CHECK_RETURN_MSG( result->first == 2, "lower bound correct");
   } // end scope

   {
      auto  result = int_tree.upper_bound( 0);
      CHECK_RETURN_MSG( result != int_tree.end(), "upper bound 0 found");
      CHECK_RETURN_MSG( result->first == 2, "upper bound correct");
   } // end scope
   ...
   CHECK_RETURN_MSG( copy.size() == numbers.size(), "Unexpected number of elements in copy!");

   CHECK_RETURN_MSG( test_iteration_fwd( copy.begin(), copy.end()), "iterator on copy");

   CHECK_RETURN_MSG( test_iteration_bwd( copy.rbegin(), copy.rend()),
                 "reverse_iterator on copy");
...
/// the tree.
/// @tparam  I  The type of the iterator to use.
/// @param[in]  first  Iterator pointing to the first elemenz.
/// @param[in]  last   Iterator pointing behind the lasz elemen.
/// @return  \c true if the values were in the correct order.
/// @since  x.y.z, 26.03.2018
template< typename I> bool test_iteration_fwd( I first, I last)
{
   ...
   while (first != last)
   {
      CHECK_RETURN_MSG( first->first == idx, "Wrong value from iteration, expected " +
         std::to_string( idx) + ", got " + std::to_string( first->first) + "!");
...
/// @tparam  I  The type of the iterator to use.
/// @param[in]  first  Pointer to the element to start iterating with (last
///                    element).
/// @param[in]  last   Pointer to the end of the sequence (here: pointer before
///                    the first element).
/// @return  \c true if the values were in the correct order.
/// @since  x.y.z, 26.03.2018
template< typename I> bool test_iteration_bwd( I first, I last)
...
// =====  END OF test_btree_permutations.cpp  =====
