...
BOOST_AUTO_TEST_CASE( test_multi_values)
{
   ...
   // first check with the default behaviour: no multi-values
   {
      ...
      BOOST_REQUIRE_NO_THROW( ah.addArgument( "m", DEST_VAR( m), "values"));
      BOOST_REQUIRE_THROW( evalArgumentString( ah, "-s 4,four;5,five;6,six "
         "7,seven"), std::invalid_argument);
   } // end scope
   ...
   // accept multi-values, would want to add a free value, but flag is not set
   {
      ...
      BOOST_REQUIRE_NO_THROW( ah.addArgument( "-", DEST_VAR( free),
         "free value"));
      BOOST_REQUIRE_THROW( evalArgumentString( ah, "-m 4,four;5,five;6,six "
         "7,seven --endvalues 8"),std::invalid_argument);
   } // end scope
...
BOOST_AUTO_TEST_CASE( format_values)
{
   ...
   BOOST_REQUIRE_NO_THROW( evalArgumentString( ah, "-m 1,monday;2,TUESDAY;"
      "3,wEdNeSdAy"));
   BOOST_REQUIRE_EQUAL( m.size(), 3);
...
BOOST_AUTO_TEST_CASE( test_unique_values)
{
   ...
   // first check the default: duplicate values overwrite silently
   {
      ...
      BOOST_REQUIRE_NO_THROW( evalArgumentString( ah, "-v 2,two;3,three;"
         "4,four;4,five;6,six;7,seven"));
      BOOST_REQUIRE_EQUAL( m.size(), 5);
   ...
   // make much sense
   {
      ...
      BOOST_REQUIRE_NO_THROW( ah.addArgument( "v", DEST_VAR( m), "values")
         ->setUniqueData());
      BOOST_REQUIRE_NO_THROW( evalArgumentString( ah, "-v 2,two;3,three;4,four;"
         "4,five;6,six;7,seven"));
      BOOST_REQUIRE_EQUAL( m.size(), 5);
   ...
   // values
   {
      ...
      BOOST_REQUIRE_THROW( evalArgumentString( ah, "-v 2,two;4,four 6,six;"
         "7,seven"), std::runtime_error);
   } // end scope
...
BOOST_AUTO_TEST_CASE( disjoint_unordered_maps)
{
   ...
   // throw if the data in the unordered maps is not disjoint
   {
      ...
      BOOST_REQUIRE_THROW( evalArgumentString( ah, "-l 1,one;2,two;3,three "
         "-r 4,four;5,five;6,six;1,one"), std::runtime_error);
   } // end scope
