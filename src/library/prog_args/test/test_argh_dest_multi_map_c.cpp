...
**
**  Description:
**    Test program for the special handling of multi-maps by the module
...
// Boost includes
#define BOOST_TEST_MODULE ArgHandlerDestMultiMapTest
...
/// Test feature that only unique values should be stored in the multi-map.
...
BOOST_AUTO_TEST_CASE( list_arg_vars)
{
   ...
   BOOST_REQUIRE( celma::test::multilineStringCompare( std_out,
      ...
      "'-m' value type 'std::multimap<int,std::string>', destination container 'm', currently no values.\n"
      "   value 'required' (2), optional, does not take multiple&separate values, don't print dflt, no checks, 1 formats.\n"
      "\n"
      "Arguments:\n"
      "'--list-arg-vars' calls function/method 'Handler::listArgVars'.\n"
      "   value 'none' (0), optional, does not take multiple&separate values, don't print dflt, no checks, no formats.\n"
      "'-m' value type 'std::multimap<int,std::string>', destination container 'm', currently 3 values.\n"
      "   value 'required' (2), optional, does not take multiple&separate values, don't print dflt, no checks, 1 formats.\n"
      "\n"));
...
// =====  END OF test_argh_dest_multi_map_c.cpp  =====
